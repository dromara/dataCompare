...
layui.define(["element","jquery","layer"],function(exports){
	...
	Tab.prototype.refreshFirst =  function () {
		$(".clildFrame .layui-tab-item").find("iframe")[0].contentWindow.location.reload();
	};
	...
	Tab.prototype.deleteTab = function (layId) {
	...
	$("body").on("click",".top_tab li i.layui-tab-close",function(){
		...
		if($(this).parent('li').find("cite").text() === "添加博客" || $(this).parent('li').find("cite").text() === "编辑"){
			...
			layer.confirm('文章还未保存，离开后内容将会丢失', {title:'确认关闭？'}, function(index){
				layer.close(index)
				element.tabDelete("bodyTab",i).init();
				bodyTab.tabMove();
			});
		}else {
	...
	$(".closePageOther").on("click",function(){
		...
		}else if($("#top_tabs li.layui-this cite").text()=="文章管理" && $("#top_tabs li").length>1){
			$("#top_tabs li").each(function(){
				if($(this).attr("lay-id") != '' && !$(this).hasClass("layui-this")){
					element.tabDelete("bodyTab",$(this).attr("lay-id")).init();
